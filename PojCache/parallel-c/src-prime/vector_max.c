...
#define baseScope	1000

#define maximumPrimes	2000000
int64_t search_scope_up_bound =30000000;     // 小于N的素数个数；   

int64_t totalPrimes = 0;
int64_t primeSize = baseScope;
int64_t unsearchedScope_low_bound = baseScope+1;
int64_t task_id = 0, result_id = 0;
...
void* searchPrimes(void *n)
{
	...
	while (1)
	{
		...
		if (loc_up_bound > search_scope_up_bound)
			loc_up_bound = search_scope_up_bound;
...
int64_t isPrime(int64_t number)
{
	...
	for (k = 0; primes[k]*primes[k] < number; k++)
	{
...
int64_t* prime_vector(int64_t start)
{
	...
	if (NULL == primes)
	{
		primes = (int64_t*) calloc(maximumPrimes, sizeof(int64_t));
		primes[0] = 2;
		totalPrimes = 1;
	...
	//printf("totalPrimes=%lld", totalPrimes);

	if (cntPrimes >= g_length)
		return vector;

	last = vector[cntPrimes-1] + 2;
	while (cntPrimes < g_length)
	{
		if (isPrime(last))
		{
			vector[cntPrimes++] = last;
		}
...
void ascMerge(int64_t* array, int64_t threadNum) 
{
	...
	for(i=0; i<threadNum; i++)
	{
		...
		for (v = 0; v < slice; v++)
		{
			if (index[i] + v < g_length)
				printf("%lld ", array[index[i]+v]);
		}
		printf("\n");
		*/
	}
...
int64_t ascCmp(const void *a, const void *b)
{
	int64_t p = *(int64_t *)a;
	int64_t q = *(int64_t *)b;
...
void* aQsort(void *args)
{
	...
	/*
	printf("before sort[%lld, %lld]:\n", s, e);
	for (i = s; i < e; i++)
	{
		printf("%lld ", node->vector[i]);
	}
	printf("\n");
	*/
...
int64_t dscCmp(const void *a, const void *b)
{
	int64_t q = *(int64_t *)a;
	int64_t p = *(int64_t *)b;
...
int64_t* descending(int64_t* vector) {
    ...
    */
	...
	for (i = 0; i < threadNum; i++)
	{
		...
		//pthread_create(&threads[i], NULL, quicksort, &threadArgs[i]);
		pthread_create(&threads[i], NULL, dQsort, &threadArgs[i]);
	}
