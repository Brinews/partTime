...
public:
	...
	*/
	...
	/* sort functions */
	int ps_cmpbyname(PeopleInfo_t* a, PeopleInfo_t *b);
...
* List Functions 
****************************************************************/
int DoubleLink::p2_cmpbytelno(PeopleInfo_t *p, PeopleInfo_t *q)
{
...
void DoubleLink::p2_insertlist(PeopleNode_t **head, PeopleInfo_t *p)
{
	if (NULL == *head)
	{
...
void DoubleLink::p2_copy2list(PeopleNode_t **head, PeopleInfo_t *arr, int siz)
{
	for (int i = 0; i < siz; i++)
	{
...
void DoubleLink::p2_timecopy(PeopleNode_t **head, PeopleInfo_t *arr, int siz)
{
	double start, end;
	start = get_cpu_time();
...
void DoubleLink::p2_printlist(PeopleNode_t *head)
{
	PeopleNode_t *h = head;
...
void DoubleLink::p2_deletelist(PeopleNode_t **head, PeopleNode_t *p)
{
	PeopleNode_t *m, *n;
...
void DoubleLink::p2_freelist(PeopleNode_t **head)
{
...
void DoubleLink::p2_testdelete(PeopleNode_t **head)
{
	int tn;
 ...
 * Return@(r)
 */
unsigned int DoubleLink::ps_random_in_range(int x, int y)
{
	int rd = rand()%(y-x+1);
 ...
 * Ret: PeopleInfo_p *p
 */
PeopleInfo_p DoubleLink::ps_readin(char *filename)
{
	char outfile[NAMLEN];
...
void DoubleLink::ps_print(PeopleInfo_p p, ostream& out)
{
	char telstr[TELSTR];
 ...
 * Ret: null
 */
void DoubleLink::ps_writeout(PeopleInfo_t *arr, int siz, char *filename)
{
 ...
 * Ret: PeopleInfo_t a
 */
PeopleInfo_p DoubleLink::ps_namesearch(PeopleInfo_t *arr, int siz, char *name)
{
 ...
 * Param@(int[3], char *)
 */
void DoubleLink::ps_telno2string(unsigned int *telno, char *str)
{
 ...
 * Return: PeopleInfo_t
 */
PeopleInfo_p DoubleLink::ps_telsearch(PeopleInfo_t *arr, int siz, char *telno)
{
	PeopleInfo_t *ret = NULL;
	char telstr[TELSTR];
 ...
 * Ret: null
 */
void DoubleLink::ps_testsearch(PeopleInfo_t *arr, int siz)
{
	char buf[NAMLEN];
 ...
 */
int DoubleLink::ps_findmax(PeopleInfo_t *arr, int s, int e)
{
 ...
 */
int DoubleLink::ps_nameorder(PeopleInfo_t *arr, int s, int e)
{
 ...
 * Ret : null
 */
void DoubleLink::ps_swap(PeopleInfo_t *arr, int left, int right)
{
	PeopleInfo_t tmp;
 ...
 * Ret: arr
 */
PeopleInfo_p DoubleLink::ps_dosort(PeopleInfo_t *arr, int siz)
{
	int max = 0;
 ...
 * selection sort
 */
void DoubleLink::ps_selectionsort(PeopleInfo_t *arr, int siz)
{
	int max = 0;
...
int DoubleLink::ps_cmpbyname(PeopleInfo_t *p, PeopleInfo_t *q)
{
	int l, r;
 ...
 * insertion sort
 */
void DoubleLink::ps_insertionsort(PeopleInfo_t *arr, int siz)
{
	int i, j;
 ...
 */
void DoubleLink::ps_impinsertsort(PeopleInfo_t *arr, int siz)
{
	int i, j;
	PeopleInfo_t tmp;
 ...
 * Bubble sort 
 */
void DoubleLink::ps_bubblesort(PeopleInfo_t *arr, int siz)
{
	int i, j;
 ...
 */
void DoubleLink::ps_twosidebubblesort(PeopleInfo_t *arr, int siz)
{
...
double DoubleLink::get_cpu_time()
{
#ifdef UNIX_ENV
	struct rusage ruse;
 ...
 * Ret: milliseconds
 */
double DoubleLink::ps_timecost(PeopleInfo_t *arr, int siz, char *fname)
{
	double start, end;
	char pname[NAMLEN];
 ...
 * Ret : null
 */
void DoubleLink::ps_doclear(PeopleInfo_t *arr)
{
	delete []arr;
	arr = NULL;
}
...
int main(int argc, char **argv)
{
	...
	while (cin >> fname)
	{
		...
		PeopleInfo_t *arr = dl.ps_readin(fname);

		PeopleNode_t *head = NULL;

		/* time cost for copy array to list */
		cout << "\n\n=============After Copy=============\n\n";
		dl.p2_timecopy(&head, arr, dl.getsize());
		dl.p2_printlist(head);

		/* test delete */
		cout << "\n\n------------After Delete-------------\n\n";
		dl.p2_testdelete(&head);
		dl.p2_printlist(head);
